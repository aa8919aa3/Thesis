...
SQUID 作為巨觀量子工程的典範代表，運用超導性的基本原理來創造已知最靈敏的磁通量探測器。從完全對稱的理想化模型到具有工程化非對稱性的先進配置，SQUID 展現了從簡單干涉儀到可調式、非互易量子元件的演化過程。本章將建立一個全面的理論框架，從超導性的基本概念出發，逐步發展到現代非對稱 SQUID 的複雜物理學。

== 超導體中的宏觀量子現象基礎 (Foundations of Macroscopic Quantum Phenomena in Superconductors)

要理解 SQUID 的原理，必須先深入探討超導性 (superconductivity) 這個深刻且往往違反直覺的世界，這是一種物質狀態，其特徵在於集體量子相干性貫穿材料的整個物理維度。

=== 庫柏對凝聚與宏觀波函數 (The Cooper Pair Condensate and the Macroscopic Wavefunction)

當一種材料冷卻到其特有的臨界溫度 $ T_C $ 以下。

進入超導狀態時，這代表了在凝聚態物理中觀察到的最引人注目的相變之一。此現象的微觀起源由巴丁-庫柏-施里弗 (Bardeen-Cooper-Schrieffer, BCS) 理論在 1957 年闡明，至今仍是我們理解傳統超導性的基石。根據 BCS 理論，在溫度低於 $ T_C $ 時，電子之間一種有效的、儘管微弱的吸引力可以克服它們之間的庫倫排斥力。這種吸引力是由材料的晶格振動，即聲子 (phonons)，所介導的。一個電子穿過晶格時會使正離子晶格變形，產生一個瞬時的正電荷區域，從而吸引第二個電子。這種聲子介導的交互作用導致兩個動量與自旋相反的電子形成束縛態，這個量子力學實體被稱為庫柏對 (Cooper pair)。
...
是它能主動將磁通量從其內部排出，此現象稱為邁斯納效應 (Meissner effect)。這將超導體與假設的「完美導體」（僅具零電阻的材料）區分開來。當一個材料在外部磁場存在下被冷卻至其臨界溫度 $T_C$ 以下時，它不僅僅是像完美導體那樣捕獲已存在的磁通線，而是會在表面產生持久的、無耗散的電流，稱為屏蔽電流 (screening currents)。
...
進一步地，對於一個厚的超導環（其厚度遠大於磁穿透深度 (magnetic penetration depth)），積分路徑 C 可以選擇在超導材料深處，那裡的屏蔽電流已衰減為零，即 $arrow(J)_s = 0$。在這個重要的極限下，磁通量量子化條件簡化為穿過迴路的磁通量 $Phi$ 本身的量子化。對向量位項應用斯托克斯定理 (Stokes' theorem) ($integral.cont_C arrow(A) dot d arrow(l) = integral_S (nabla times arrow(A)) dot d arrow(S) = integral_S arrow(B) dot d arrow(S) = Phi$)，條件變為：
$ Phi = n Phi_0 $
...
SQUID 的基本主動元件是約瑟夫森接面 (Josephson junction, JJ)，這是一種在兩個超導電極之間建立「弱連結」(weak link) 的裝置。此效應最初由布萊恩·約瑟夫森 (Brian Josephson) 在 1962 年從理論上預測，描述了庫柏對穿過此連結的相干量子力學穿隧 (coherent quantum mechanical tunneling)。

=== 第一及第二約瑟夫森關係 (The First and Second Josephson Relations)

約瑟夫森接面可以透過多種方式實現，但最典型的例子是超導體-絕緣體-超導體 (superconductor-insulator-superconductor, S-I-S) 穿隧接面，其中兩個超導薄膜被一層非常薄的絕緣障壁隔開，厚度通常只有幾奈米。在古典物理中，這個障壁在低電壓下會阻止任何電流通過。然而，在量子力學中，兩個超導體中庫柏對凝聚體的宏觀波函數可以穿透絕緣區並發生重疊。這種波函數的「鎖定」在障壁上建立了一個相位相關的連結，使得庫柏對能夠在不被破壞的情況下穿隧。這個相干穿隧過程由兩個基本方程式描述。

*第一約瑟夫森關係 (First Josephson Relation)*，也稱為直流約瑟夫森效應 (DC Josephson effect)，指出在完全沒有外加電壓的情況下，一個無耗散的超導電流 $I_s$ 可以流過接面。此超導電流的大小和方向由兩個超導電極宏觀波函數之間的規範不變相位差 (gauge-invariant phase difference) $phi = phi_2 - phi_1$ 決定。對於一個簡單的穿隧接面，此關係呈現正弦形式：
$ I_s = I_c sin(phi) $
...
*第二約瑟夫森關係 (Second Josephson Relation)*，或稱交流約瑟夫森效應 (AC Josephson effect)，描述了當在接面上維持一個直流電壓 V 時，相位差隨時間的演變。相位差不是靜態的，而是根據以下公式隨時間演變：
...
同時，標準正弦電流-相位關係 $I_s = I_c sin(phi)$ 是理想 S-I-S 穿隧接面的典型形式，並作為理解基本 SQUID 操作的基礎模型。此形式源於單一庫柏對穿隧障壁的最低階量子力學過程。然而，隨著材料科學和奈米製造技術的進步，已開發出多種多樣的約瑟夫森接面，每種都展現出獨特的 CPR 特性。
...
直流 SQUID (DC SQUID) 是一種巧妙利用磁通量量子化與約瑟夫森接面電流-相位關係的裝置，透過將兩個約瑟夫森接面並聯在一個超導環上，創造一個磁通量量子干涉儀。

=== 對稱直流 SQUID 的理論模型 (Theoretical Model of Symmetric DC SQUIDs)

直流 SQUID，如@fig-squid-schematic 示意圖所示，由一個幾何自感為 L 的閉合超導迴路組成，該迴路被兩個並聯的約瑟夫森接面 JJ1 和 JJ2 中斷。偏置電流 $I_b$ 從迴路一側注入，從另一側取出，導致電流分流並流經包含接面的兩個臂。

「對稱直流 SQUID」的概念指的是一個理想化模型，其中兩個接面和迴路的物理參數完全匹配：
...
雖然沒有任何真實裝置是完美對稱的，但這個模型提供了量子干涉的基本物理，並作為理解不對稱性和其他非理想性影響的基準。
...
*磁通量調變深度*：對於對稱 SQUID，磁通量調變的深度為 100%，即在半整數磁通量量子處臨界電流可以完全抵消。而對於非對稱 SQUID，調變深度由非對稱參數決定：
...
這種行為將 SQUID 從一個簡單的感測器轉變為一個強大的表徵工具。對稱 SQUID 的磁通調變模式充當其組成接面電流-相位關係的一種傅立葉光譜學 (Fourier spectroscopy)。
...
本章提供了一個從基礎原理到先進應用的超導量子干涉儀全面理論框架。從宏觀量子力學的基本概念出發，我們建立了理解 SQUID 運作的核心物理原理：庫柏對凝聚體的宏觀波函數、邁斯納效應、磁通量量子化，以及約瑟夫森效應。

理論發展的核心論點是，組成約瑟夫森接面的電流-相位關係至關重要。CPR 不僅僅是一個描述性參數，而是作為弱連結中電荷輸運的微觀物理與整個裝置的宏觀、可測量量子行為之間的根本聯繫。

對於對稱直流 SQUID，磁通量量子化和約瑟夫森效應的原理相結合，產生了宏觀量子干涉現象。總超導電流受到穿過 SQUID 迴路的磁通量的調變，導致臨界電流以磁通量量子 $Phi_0$ 的周期週期性地變化。這種調變是庫柏對波函數穿過 SQUID 兩臂的干涉圖案，是雙縫光學實驗的直接類比。

非對稱性的引入開啟了更豐富的物理現象。非對稱 SQUID 不僅保持了基本的量子干涉特性，更成為探測接面內在電流-相位關係的強大工具。透過分析磁通調變圖樣的畸變，可以重構出單個接面的 CPR，並識別非傳統的輸運機制。

非正弦 CPR 的探索突顯了該領域的現代視角。標準的正弦關係現在被理解為更通用的多諧波描述的低透明度極限。接面 CPR 中高次諧波的存在導致 SQUID 中複雜、非餘弦的磁通調變模式，將裝置轉變為能夠探測其自身組件諧波含量的光譜工具。

該領域的未來展望與電流-相位關係的持續探索和工程化密切相關。材料科學的進步，包括基於半導體、石墨烯和拓撲絕緣體的混合接面的開發，有望提供對 CPR 的前所未有的控制。在這些新穎系統中對 CPR 的精確測量和理論理解，通常使用基於 SQUID 的技術，對於釋放其潛力至關重要。
...
這些發展將進一步推動超導量子電子學和拓撲量子計算領域的進步。曾經主要作為感測器的 SQUID，將繼續發展成為一個不可或缺的平台，用於研究基本量子輸運和構建下一代超導電子學和量子資訊處理器。
