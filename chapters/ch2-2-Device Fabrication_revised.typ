// Revision Summary
// Date: 2025-08-05
// Chapter: ch2-2-Device Fabrication
// Total revisions: 6
// Critical fixes: 0
// Improvements: 5
// Suggestions: 1
...
此外，本章將詳細說明本實驗使用機械剝離法 (Mechanical Exfoliation Method) 得到二維材料層狀薄片 (Flake) 的完整流程與方法，包括基板預處理、膠帶黏貼剝離。
...
此外，本實驗使用的基板為低電阻重參雜 P 型單面矽/熱氧化二氧化矽基板 (NOVA electronics \#HS39626-OX：4”, P-type, <100>, 1.0∼ 5.0 × 10−3Ω· 𝑐𝑚 Resistivity, 2850 A°±5% Dry Thermal Oxide)。

此外，1.  將 7×7mm 大小的破片放入 140 °C 的二甲基亞碸 (DMSO, Dimethyl sulfoxide) 溶劑中靜置 1 小時，以去除基板上保護用的 S1813 正型光阻薄膜。
...
此外，本實驗使用的膠帶為 Nitto SPV-224PR-MJ，以下簡稱藍膠。

此外，1.  確認手套箱內水氧值皆小於 0.5ppm，將零號藍膠 (Tape 0) 膠面朝上固定於平面，用鑷子將夾取 PtTe2 塊材 (Bulk) 放置於 Tape 0 上。
...
9.  儘速將基板移動至旋轉塗佈機 (Spin Coater) 使用 PMMA A6 以 500 rpm 旋轉 5 秒, 以高加速度升速至最終轉速 4000 rpm，此旋塗步驟總時長為 55 秒。 // REVISED: Line-specific - Check for repeated articles ('the the' or 'a a')...
10. 使用 180 °C 預烤 (Prebake) 基板 3 分鐘即完成機械剝離流程。

== 顯微鏡檢測 (Microscopy)

此外，本章將詳細說明本實驗使用的各種顯微鏡及其目的與使用方式。

=== 光學顯微鏡 (Optical Microscopy)

此外，本實驗使用的光學顯微鏡 (OM, Optical Microscope) 為 KEYENCE VHX-7000N，使用目的主要有以下幾種：

此外，1.  *尋找與定位二維材料層狀薄片 (Flake)*：完成機械剝離流程後，透過光學顯微鏡 (OM, Optical Microscope) 初步判斷是否有 PtTe2 薄片，並使用 VHX-7000N 的連續拍攝功能拍攝基板的四個角落及薄片照片，並得到座標。
...
此外，本實驗使用的原子力顯微鏡 (AFM, Atomic Force Microscopy) 為 Park systems NX10，用以判斷目標 PtTe2 薄片的層數，與確認接觸金屬的厚度與形貌。

此外，為了對元件的微觀結構與形貌進行精確表徵，本研究採用了一台 *Park Systems NX10 原子力顯微鏡* 進行量測。AFM 在此研究中扮演了兩個關鍵角色：(1) 鑑定機械剝離 (mechanically exfoliated) 的 PtTe₂ 薄片的厚度與層數；(2) 確認蒸鍍金屬電極的厚度、表面粗糙度與邊緣形貌。

此外，所有 AFM 量測均在*非接觸模式 (non-contact mode)* 下進行，以最小化探針與樣品表面之間的交互作用，從而保護脆弱的二維材料薄片與金屬結構不被物理損傷。

此外，首先，對於 PtTe₂ 薄片，我們透過量測其表面與周圍 SiO₂ 基板之間的*步階高度 (step height)* 來確定其物理厚度。基於已知的 PtTe₂ 材料層間距，我們可以從量測到的厚度推算出薄片的原子層數。這一步驟對於篩選出符合實驗需求的特定層數（例如單層或少層）的樣品至關重要。
...
此外，本研究中所有奈米級圖案的定義，例如對準標記與元件電極，均是透過電子束微影技術完成。整個流程在一台 Raith PIONEER Two 掃描式電鏡/電子束微影系統上執行。以下將詳述其製備、曝光與顯影的具體步驟。
...
此外，抗蝕劑準備完成後，樣品被送入 EBL 系統的真空腔體中進行圖案曝光。所有的曝光步驟均在 20 kV 的加速電壓下進行。此流程同樣分為兩步：

此外，1.  *對準標記曝光*：第一步是在樣品上定義大面積的對準標記 (alignment markers)，這些標記將作為後續高精度圖案疊對的基準。為實現快速曝光，此步驟選用了 120 µm 的大光圈 (aperture) 以獲得約 12 nA 的較大電子束流，並採用 370 μC/cm² 的面積劑量 (area dose) 進行寫入。
...
此外，為了能夠在後續的電子束微影 (EBL) 步驟中，將奈米級的電極圖案精確地疊對 (align) 到目標 PtTe₂ 薄片上，我們首先在 SiO₂/Si 基板上製作了一組金屬材質的全域對準標記 (global alignment markers)。

此外，標記的圖案首先透過 EBL 技術被定義在抗蝕劑層上。在完成顯影後，樣品被送入一台 *Korvus HEX 桌上型物理氣相沉積 (PVD) 系統*中進行金屬沉積。該系統的核心是一個四腔電子束蒸發器，其原理是利用高能電子束在真空環境下直接轟擊並加熱裝有源材料的坩堝，使材料蒸發。

此外，為確保標記的穩定性與高對比度，我們採用了雙層金屬結構。在腔體被抽至高真空環境後，依序蒸鍍：

此外，1.  *附著層 (Adhesion Layer)*：一層約 10 nm 厚的*鉻 (Cr)*，用以增強標記與基板的附著力。
2.  *對比層 (Contrast Layer)*：一層約 50 nm 厚的*金 (Au)*，其較高的原子序數能夠在掃描式電子顯微鏡 (SEM) 下提供清晰的二次電子或背向散射電子圖像，從而利於 EBL 系統進行自動識別和對位。

此外，金屬沉積完成後，透過在丙酮中進行*離舉 (lift-off)*，移除抗蝕劑，最終在基板表面留下永久性的 Cr/Au 金屬對準標記。這些標記為所有後續的材料轉移和微影製程提供了一個固定的、高精度的參考坐標系。

== 濺鍍沉積 (Sputter Deposition)

此外，最終用以連接 PtTe₂ 元件的超導電極，是透過一套包含原位 (in-situ) 介面清潔與磁控濺鍍 (magnetron sputtering) 的整合性製程完成。所有步驟均在 Aja Orion 8 物理氣相沉積 (PVD) 系統中執行，以確保最佳的介面品質。
...
此外，介面清潔完成後，樣品維持在高真空環境中，不破壞真空，立即進行超導薄膜的沉積。我們選擇*鈮鈦 (NbTi)* 合金作為超導接觸電極材料。濺鍍製程在 3 mTorr 的氬氣工作壓力下進行。一個 300 W 的直流 (DC) 電源被施加於 NbTi 靶材上，使其原子被濺射並沉積到樣品表面，最終形成一層厚度為 70 nm 的均勻薄膜。
