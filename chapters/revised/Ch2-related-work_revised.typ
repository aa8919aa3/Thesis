...
塊材二碲化鉑本身並沒有超導性，然而，在本研究所探討的約瑟夫森接面元件中，二碲化鉑 作為「弱連結 (weak link)」區域，其超導性主要是透過與具有更高超導轉變溫度的電極（如鈮鈦合金 Niobium–titanium, Nb-Ti）接觸，經由超導近接效應 (superconducting proximity effect) 所誘導產生。這就構成了一個超導體-拓撲半金屬-超導體 (Superconductor–Topological Semimetal–Superconductor, S-TS-S) 的混合結構。
...
進一步地，約瑟夫森常數（Josephson constant）的定義為$K_J=frac(2e, h)$，其倒數則為磁通量子（magnetic flux quantum）$Phi_0=frac(h, 2e)=2pi frac(hbar, 2e)$。則超導相變化方程式@eq-2nd-josephson 可以重新表達為：
...
同時，假設在時間 $t_1$ 時，約瑟夫森相為 $phi_1$ ；在較晚的時間 $t_2$ ，約瑟夫森相演變為 $phi_2$ 。接面的能量增加等於對面點所做的功：
