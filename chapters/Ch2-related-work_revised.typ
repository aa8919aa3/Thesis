...
此外，本章節回顧與本研究相關的理論基礎和先前研究成果。相關研究可以分為以下幾個主要方向：
...
此外，約瑟夫森接面 (Josephson Junction, JJ) 是構成超導量子電路的基本構建模組。其物理結構為由一個「弱連結 (weak link)」隔開的兩個超導體。這個弱連結可以是一個極薄的絕緣層 (S-I-S)、一段正常金屬 (S-N-S)，或如本研究中的拓撲半金屬 (S-TS-S)。儘管庫柏對 (Cooper pairs) 無法在弱連結材料中長距離穩定存在，但它們可以透過量子穿隧 (quantum tunneling) 或近接效應 (proximity effect) 的方式，發生約瑟夫森效應即在弱連結區域建立起相干的超導電流的宏觀量子現象。
此效應由 Brian Josephson 於 1962 年預測 @JOSEPHSON1962251 ，並由兩個支配接面量子動力學的基本關係式所描述。

此外，第一約瑟夫森關係式（first Josephson relation）@barone1982physics，闡述了流經接面的無耗散超導電流 $I_s$ 與兩端超導體波函數的相位差 $phi$ 之間的關係，這便是電流-相位關係 (Current-Phase Relation, CPR)。它指出，在無任何電壓的情況下，超導電流$I_s$仍可流過接面，其驅動力完全來自於兩個超導電極中序參數 (order parameters) 的宏觀量子相位差$phi$@feynman_feynman_2011，對於傳統的「超導體-絕緣體-超導體」(SIS) 穿隧接面，此關係呈現為一簡單的正弦形式，即直流約瑟夫森效應（The DC Josephson effect）：

$ 
I_(s)(phi) = I_c sin(phi(t))","
$ <eq-1st-josephson>

此外，其中 $ phi = phi_2 - phi_1 $ 為金兹堡-朗道序參數（Ginzburg–Landau order parameter）在接面處的相位差，即約瑟夫森相位（Josephson phase）， $ I_s $ 是接面的切換電流（有時也稱跳躍電流）為外加偏壓超過某一門檻時，元件由超導態跳變至電阻態時的電流值，而 $ I_c $ 是接面在完全維持超導態（無電阻）時所能承受的最大電流，即臨界電流 (critical current)，其取決於超導體的性質，亦會受到溫度以及外加磁場等環境因素的影響。此正弦形式是一種理想化模型，許多類型的接面展現出更複雜的非正弦 CPR，這在非對稱系統的研究中是至關重要的課題 [需引用討論非正弦CPR的相關研究]。

此外，第二約瑟夫森關係式（second Josephson relation）或超導相演變方程式（superconducting phase evolution equation）@barone1982physics，描述了在接面兩端存在電壓$V$時，相位差的演化，即交流約瑟夫森效應（The AC Josephson effect）：

$ 
(partial phi)/(partial t) = (2e V(t))/hbar","
$ <eq-2nd-josephson>

此外，其中 $ e $ 是基本電荷 $ 1.602176634 times 10^(−19) C $ （庫倫）@jeckelmann_elementary_2019@mohr_codata_2024， $ hbar $ 是約化普朗克常數（reduced Planck constant） $ 1.054571817... times 10^(−34) J dot s $ （焦耳 $ dot $ 秒）@mohr_codata_2024。

此外，約瑟夫森常數（Josephson constant）的定義為$K_J=frac(2e, h)$，其倒數則為磁通量子（magnetic flux quantum）$Phi_0=frac(h, 2e)=2pi frac(hbar, 2e)$。則超導相變化方程式@eq-2nd-josephson 可以重新表達為：
...
此外，換句話說，即在接面處施加固定電壓$V_(D C)$時，相位將隨時間線性變化，而電流則為幅度為$I_c$、頻率$f$為$K_J V_(D C)$的正弦交流電。
...
此外，接著，應用連鎖律（chain rule）計算電流的時間導數：
...
此外，將上述結果整理為電感器的電流-電壓特性形式：

$
  V = Phi_0/(2pi I_c cos phi) (partial I) / (partial t)= L(phi) (partial I) / (partial t).
$

此外，這便得到動態電感的表達式，其為約瑟夫森相位的函數：

$
  L(phi) = Phi_0/(2pi I_c cos phi) = L_J/(cos phi).
$

此外，其中$L_J=L(0)=Phi_0/(2pi I_c)$為約瑟夫森接面的一個特性參數，稱為約瑟夫森電感。


此外，基於約瑟夫森接面與非線性電感的相似性，當超流經約瑟夫森接面時，其儲存的能量可以計算。@tinkham_introduction_2015
...
此外，假設在時間 $t_1$ 時，約瑟夫森相為 $phi_1$ ；在較晚的時間 $t_2$ ，約瑟夫森相演變為 $phi_2$ 。接面的能量增加等於對面點所做的功：
...
此外，依據戴維寧定理（Thévenin's theorem）@bhlpage4797088 @johnson_origins_2003， 接面的交流阻抗可由一個電容與一個並聯電阻來表示，這兩者均與理想約瑟夫森接面並聯@barone_physics_1996。電流驅動的完整表達式 $I_("ext")$變為：

$
  I_("ext") = I_s + I_R + I_C = I_c sin(phi) + V/R + C_J (dif V)/(dif t).
$

此外，其中$I_C$項為接面等效電容（effective capacitance）$C_J$帶有的位移電流（displacement current），$I_R$項為接面等效電阻（effective resistance）$R$帶有的正常電流（normal current）。


此外，根據克希荷夫電路定律（Kirchhoff Circuit Laws），流過接面的總電流 $ I $ 可寫為：
$ I = I_s + I_R + I_C $. <eq-rcsj-kcl>

此外，其中 $ I_s = I_c sin(phi) $ 為超導電流，$ I_R = V / R $ 為流過電阻的電流，$ I_C = C (dif V) / (dif t) $ 為流過電容的電流。將 $ V = (hbar) / (2 e) (dif phi) / (dif t) $ 代入，得到 RCSJ 模型核心方程：

Furthermore, $ I = I_c sin(phi) + (hbar) / (2 e R) (dif phi) / (dif t) + (hbar C) / (2 e) (dif^2 phi) / (dif t^2) $. <eq-rcsj-core>

此外，此方程為非線性微分方程，描述約瑟夫森接面在施加電流下的動態行為。RCSJ 模型能良好解釋約瑟夫森接面的 I–V 特性，包括遲滯（hysteresis）與電壓跳變（voltage jumps）。
...
此外，RCSJ 模型是理解 DC SQUID 工作原理的基礎，因 SQUID 的響應直接取決於內部約瑟夫森接面的動態行為。透過分析此模型，可預測 SQUID 的電壓響應、臨界電流調製與雜訊特性。

=== 約瑟夫森穿透深度

此外，約瑟夫森穿透深度(Josephson penetration depth)描述了外加磁場穿透長約瑟夫森接面之典型長度。它通常以$lambda_J$ 表示，並由下列公式給出：

$
  lambda_J = sqrt(Phi_0/(2pi mu_0 d' j_c)),
$

此外，其中$Phi_0$為磁通量量子，$j_c$為臨界超導電流密度（critical supercurrent density, $A/m^2$ ），而$d'$則代表超導電極的電感@weihnacht_influence_1969：
...
此外，其中$d_I$為約瑟夫森勢壘（Josephson barrier）的厚度（通常為絕緣體），$d_1$ 和$d_2$ 分別為超導電極的厚度，而$lambda_1$和 $lambda_2$則為其倫敦穿透深度（London penetration depths）。若臨界電流密度非常低，約瑟夫森穿透深度通常在數微米至數毫米之間@buckel_supraleitung:_2004。


=== 超導量子干涉儀 (SQUID) <subsection-SQUID>

此外，超導量子干涉儀（Superconducting Quantum Interference Device, SQUID）是基於約瑟夫森效應的超導元件，具有極高的磁場靈敏度和精確度，其工作原理基於磁通量量子化和量子干涉效應。RF SQUID 只需要一個約瑟夫森接面（超導穿隧接面），這可能使其生產成本較低，但靈敏度較低。DC。SQUID 主要有兩種類型：直流（DC）和射頻（RF）。DC SQUID 的基本結構由兩個約瑟夫森接面組成，這些接面被一個薄材料所隔開形成弱連結。RF SQUID 只需要一個約瑟夫森接面（超導穿隧接面），這可能使其生產成本較低，但靈敏度較低。

此外，直流超導量子干涉儀（DC SQUID）於 1964 年由福特研究實驗室（Ford Research Labs）的 Robert Jaklevic、John J. Lambe、James Mercereau 和 Arnold Silver 所發明，此後於 1962 年由 Brian Josephson 提出約瑟夫森效應，並於 1963 年由 John Rowell 和 Philip Anderson 在貝爾實驗室（Bell Labs）製造了第一個約瑟夫森接面@anderson_probable_1963。它在超導迴路中並聯了兩個約瑟夫森接面。它基於直流約瑟夫森效應。在沒有任何外部磁場的情況下，輸入電流（input current）$I_("input")$會平均分佈到兩個分支。若對超導迴路施加一個小的外部磁場，則會開始在迴路中循環一個屏蔽電流（screening current）$I_s$，該電流會產生一個抵銷外加磁通量的磁場，並創造一個與外部磁通量成正比的額外約瑟夫森相位。感應電流（induced current）的方向與$I$在超導迴路的一個分支中相同，而在另一個分支中與 $I$ 相反；總電流變為 $1/2I_("input")+I_s$ 在某一分支，而$1/2I_("input")-I_s$ 在另一分支。一旦任一分支中的電流超過約瑟夫森接面的臨界電流$I_c$，則在接面上會出現電壓。
...
此外，若輸入電流大於$I_c$，則 SQUID 始終運作於電阻模式。此時，電壓即為外加磁場的函數，且週期等於$Phi_0$。由於直流 SQUID 的電流-電壓特性（current-voltage characteristic）具有遲滯現象（hysteretic），故在接面間連接一個並聯電阻$R$以消除遲滯（在內稟電阻（intrinsic resistance）足夠的情況下）。屏蔽電流為應用磁通量除以環路的自感。因此$Delta Phi$可估算為$Delta V$磁通量至電壓轉換器）@de_lacheisserie_magnetism:_2002 @clarke_squid_2004 ，如下所示：
...
此外，其中$i_c$為SQUID的臨界電流。通常$lambda$為一個數量級@de_waele_quantum-interference_1969。

=== 磁通量量子化

此外，在超導環中，磁通量是量子化的；其基本磁通量量子（magnetic flux quantum）為：
$ Phi_0 = h / (2 e) approx 2.067 × 10^-15 $ Wb. <eq:phi0>

此外，因此環中磁通量 $Phi$ 只能取整數倍：$Phi = n Phi_0$，其中 $n$是一個整數。這個量子化條件是 SQUID 能夠測量微弱磁場的根本原因。

=== DC SQUID 的工作機制：超導電流的量子干涉

此外，DC SQUID 優雅地利用了前述的兩個基本原理——超導迴路中的磁通量量子化與約瑟夫森接面的電流-相位關係——來創造一個磁通量的量子干涉儀。透過在超導環上並聯兩個約瑟夫森接面，該元件能透過兩個超導電流路徑的量子力學干涉，對穿過迴路的磁通量變得敏感。本節將對理想對稱 DC SQUID 的總電流-相位關係進行形式推導，並詳細分析此干涉如何導致元件臨界電流的週期性調變，這正是其作為超靈敏磁力計功能的基礎。
...
此外，DC SQUID 的結構如圖 @fig-squid-schematic 所示，由一個幾何自感為 $L$ 的封閉超導迴路組成，該迴路被兩個並聯的約瑟夫森接面 JJ1 和 JJ2 中斷。偏置電流 $I_b$ 從迴路一側注入，從另一側取出，使電流分流並流經包含接面的兩個臂。

此外，*對稱 DC SQUID* 的概念指的是一個理想化模型，其中兩個接面和迴路的物理參數完全匹配。具體來說，假設滿足以下條件：
...
此外，雖然沒有任何真實元件是完美對稱的，但這個模型提供了量子干涉的基本物理原理，並作為理解不對稱和其他非理想性影響的基準。SQUID 的行為由一組關鍵參數決定，這些參數結合了接面的內在屬性與迴路的幾何形狀。

==== 總電流-相位關係的形式推導

此外，DC SQUID 的核心特性是其總電流-相位關係，它描述了總超導電流如何依賴於有效相位差和磁通量。推導過程始於對稱 SQUID 迴路應用克希荷夫電流定律和磁通量量子化條件。

此外，假設接面具有標準的正弦 CPR，則流經兩個臂的超導電流 $I_1$ 和 $I_2$ 為：
...
此外，為了簡化總電流的表達式，我們使用三角恆等式 $sin A + sin B = 2 sin((A+B)/2) cos((A-B)/2)$。代入磁通量量子化條件中的 $phi_1$ 和 $phi_2$ 表達式，可得：
...
此外，前述對理想對稱 DC SQUID 的分析提供了量子干涉的基本物理原理。然而，真實 SQUID 的性能和行為受到非理想因素的顯著影響，最主要的是超導迴路的有限電感和熱漲落的影響。一個全面的理論模型必須考慮這些效應，才能準確描述元件的動態並預測其最終靈敏度。電阻電容並聯（RCSJ）模型為納入這些非理想性提供了一個強大的框架，將 CPR 的靜態圖像轉變為在實際操作環境中 SQUID 相位演化的動態描述。
...
此外，當 SQUID 的偏置電流 $I_b$ 超過其與磁通量相關的臨界電流 $I_("max")(Phi)$ 時，它會轉變為電阻態，並在其端子間出現有限電壓。為了模擬這種動態耗散行為，我們採用了*電阻電容並聯（RCSJ）模型*。在此模型中，每個理想的約瑟夫森元件（由其 CPR 描述）與一個電阻 $R_N$（代表準粒子穿隧）和一個電容 $C$（代表接面的幾何電容）並聯。

此外，通過對 SQUID 電路的兩個迴路應用克希荷夫定律，可以推導出一組關於相位差 $phi_1$ 和 $phi_2$ 的耦合二階非線性微分方程。在任何有限溫度下，分路電阻都會表現出約翰遜-奈奎斯特熱噪聲，這表現為與每個接面並聯的波動噪聲電流源 $I_(N i)(t)$。得到的運動方程是一組耦合的朗之萬方程：
...
此外，本分析為理解對稱 DC SQUID 中的電流-相位關係提供了一個全面的理論框架。從宏觀量子力學的基本原理到實際 SQUID 的複雜動力學的過程，揭示了一套緊密相連的物理概念。出現的核心論點是，組成約瑟夫森接面的電流-相位關係至關重要。CPR 不僅僅是一個描述性參數，而且是弱連結中電荷傳輸的微觀物理與整個元件的宏觀、可測量量子行為之間的基本聯繫。

此外，對非理想性的分析證明了實際元件參數的關鍵作用。由屏蔽參數 $beta_L$ 量化的有限迴路電感，作為一種內在回饋機制，可以屏蔽外部場並降低臨界電流的調變深度。通過 RCSJ 模型納入的熱漲落，驅動了系統的動力學，引起熱激活相滑移，從而產生 SQUID 的電壓響應和內在噪聲。

此外，對非正弦 CPR 的探索突顯了該領域的現代視角。標準的正弦關係現在被理解為更通用的多諧波描述的低透明度極限。接面 CPR 中較高諧波的存在，導致 SQUID 中出現複雜的非餘弦磁通量調變模式，將該元件轉變為能夠探測其自身組分諧波含量的光譜工具。

此外，該領域的未來展望與電流-相位關係的持續探索和工程設計密切相關。材料科學的進步，包括基於半導體、石墨烯和拓撲絕緣體的混合接面的發展，有望提供對 CPR 的前所未有的控制。對這些新型系統中 CPR 的精確測量和理論理解，對於釋放其潛力至關重要。對稱 SQUID 曾主要作為傳感器，將繼續發展成為一個不可或缺的平台，既用於研究基本的量子傳輸，也用於構建下一代超導電子學和量子信息處理器。
