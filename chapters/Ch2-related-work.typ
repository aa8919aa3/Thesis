...
DC SQUID 的兩個約瑟夫森接面並聯在一個超導環上。當一個偏置電流 $I_b$ 施加到 SQUID 上時，它會分成兩部分，分別流經兩個約瑟夫森接面。接面的總相位差 $Delta phi$ 不僅取決於施加的偏置電流，還取決於穿過超導環的總磁通量 $Phi_T$。總磁通量 $Phi_T$ 是外部施加磁通量 $Phi_a$ 和 SQUID 環路中由超導電流引起的自感磁通量 $L I_s$ 的總和，其中 $L$ 是 SQUID 環路的電感。

根據磁通量量子化條件和約瑟夫森關係式，兩個接面的相位差之和與環路中的磁通量有關。具體來說，兩個接面的相位差滿足
$
