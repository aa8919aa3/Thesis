...
根據磁通量量子化條件和約瑟夫森關係式，兩個結的相位差之和與環路中的磁通量有關。具體來說，兩個結的相位差 
ϕ
1
ϕ 
1
...
一個 DC SQUID 由兩個約瑟夫森接面並聯於同一超導環構成；施加偏置電流 $I_b$ 後，電流分別經兩個接面：
$ I_1 = I_{c1}\,\sin(\phi_1),\qquad I_2 = I_{c2}\,\sin(\phi_2). $ <eq:squid-branch>

總磁通量等於外加磁通量與自感磁通量之和：
$ \Phi_T = \Phi_a + L\,I_s, $ 其中 $L$ 為環電感、$I_s$ 為循環超導電流。 <eq:phi-total>

由磁通量量子化與規範不變相位關係，兩接面相位差之差與總磁通量滿足：
$ \phi_1 - \phi_2 = 2\pi\,\frac{\Phi_T}{\Phi_0} + 2\pi n,\quad n\in\mathbb{Z}. $ <eq:phase-constraint>

若兩接面對稱（$I_{c1}=I_{c2}=I_c$）且 $beta_L\equiv 2LI_c/\Phi_0 \ll 1$（環自感可忽略），則 SQUID 的最大零電壓超導電流（臨界電流）對外加磁通量的調製為
$ I_c^{\mathrm{total}}(\Phi_a) = 2\,I_c\,\bigl|\cos\!\bigl(\pi\,\Phi_a/\Phi_0\bigr)\bigr|. $ <eq:Ic-mod>

=== 磁通量–電壓轉換函數（Flux-to-Voltage Transfer Function）

當以固定偏置電流 $I_b$（通常略大於 $I_c^{\mathrm{total}}$ 的最小值）操作時，SQUID 兩端平均電壓 $V$ 隨外加磁通量呈 $Phi_0$ 週期變化。定義轉換函數
$ V_{\Phi} \equiv \left|\frac{\partial V}{\partial \Phi_a}\right|. $ <eq:Vphi-def>

在典型操作條件下，$V_\Phi$ 的極大值出現在 $Phi_a \approx \left(n+\tfrac{1}{4}\right)\Phi_0$ 或 $left(n+\tfrac{3}{4}\right)\Phi_0$ 的附近。 <eq:Vphi-max-note>

=== 臨界電流調製與週期性響應

偏置電流 $I_b$ 在兩接面間分配：$I_b = I_1 + I_2$。隨著 $Phi_a$ 變化，式 <eq:phase-constraint> 使 $phi_1,\phi_2$ 發生相對位移，進而調製 $I_c^{\mathrm{total}}(\Phi_a)$（式 <eq:Ic-mod>）。當 $I_b$ 大於當前磁通下的 $I_c^{\mathrm{total}}$，SQUID 進入電壓態並產生非零平均電壓；反之則保持零電壓態，因而形成 $V(\Phi_a)$ 的週期性。 

物理圖像
- 外加磁通量改變環中的超導循環電流，為滿足量子化條件（式 <eq:phase-constraint>），兩接面的相位落在不同點，造成兩支路超導電流的干涉。
- 當兩接面等效「同相」時（$Phi_a \approx n\Phi_0$），臨界電流加成達極大；當「反相」時（$Phi_a \approx (n+\tfrac{1}{2})\Phi_0$），互相抵消趨近極小，故 $I_c^{\mathrm{total}}(\Phi_a)$ 呈 $Phi_0$ 週期性起伏（式 <eq:Ic-mod>），並導致 $V(\Phi_a)$ 呈對應的週期性。
...
$ I_1 = I_(c1) sin(phi_1),  I_2 = I_(c2) sin(phi_2). $ <eq:squid-branch>
$ Phi_T = Phi_a + L I_s. $ <eq:phi-total>
$ phi_1 - phi_2 = 2 pi Phi_T / Phi_0 + 2 pi n,  n in ZZ. $ <eq:phase-constraint>

# 假設對稱（I_(c1)=I_(c2)=I_c，且 beta_L << 1）：
$ I_c^(total)(Phi_a) = 2 I_c abs(cos(pi Phi_a / Phi_0)). $ <eq:Ic-sym>

#（可選）一般不對稱情形的標準結果：
$ I_c^(total)(Phi_a) = sqrt(I_(c1)^2 + I_(c2)^2 + 2 I_(c1) I_(c2) cos(2 pi Phi_a / Phi_0)). $ <eq:Ic-asym>

= 3.3 磁通量–電壓轉換函數

$ V_Phi := abs( ∂ V / ∂ Phi_a ). $ <eq:Vphi-def>

在典型偏置下，$V(Phi_a)$ 以 $Phi_0$ 為週期，最大斜率常落在
$ Phi_a approx (n + 1/4) Phi_0 $ 或 $ (n + 3/4) Phi_0 $ 附近。 <eq:Vphi-max-note>

= 3.4 臨界電流調製與週期性響應

$ I_b = I_1 + I_2 $（見式 <eq:squid-branch>）。隨 $Phi_a$ 改變，式 <eq:phase-constraint>
強制兩結相位相對位移，導致 $ I_c^(total)(Phi_a) $ 的週期性（式 <eq:Ic-sym>），
並使 $ V(Phi_a) $ 呈對應週期性。
