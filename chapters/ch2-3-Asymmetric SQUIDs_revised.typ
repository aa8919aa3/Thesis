...
超導量子干涉儀 (SQUID) 作為巨觀量子工程的典範代表。

運用超導性的基本原理來創造已知最靈敏的磁通量探測器。儘管完全對稱的 SQUID 行為提供了量子干涉的基礎理解，但非對稱性的引入卻開啟了更豐富且更複雜的物理現象景觀。無論非對稱性是來自於不可避免的製程缺陷還是有意設計的工程考量，它都能將 SQUID 從簡單的干涉儀轉變為可調式、非互易的量子元件，其應用範圍從下一代磁力計到量子電腦的基本構成元件。
...
前一節所描述的理想直流 SQUID 模型。

其前提是構成迴路的兩個約瑟夫森接面 (Josephson Junction, J1 and J2) 的特性完全相同（即 $ I_("c1") = I_("c2") $ ），且在迴路中對稱分佈。然而，在實際的元件製備過程中，由於奈米級製程的內在變異性 (inherent variations)，兩個接面幾乎不可避免地存在差異，從而導致非對稱性 (asymmetry)。更有甚者，在許多現代量子元件的設計中，非對稱性不僅不是缺陷，反而是一種被有意引入的、用以實現特定功能的設計策略。
...
進一步地，當 $alpha = 0$ 時，系統退化為對稱 SQUID；當 $alpha != 0$ 時，系統表現出非對稱行為。
...
同時，當 $I_("c1") = I_("c2")$ 時，調變深度為 100%；當差異增大時，調變深度減小。
