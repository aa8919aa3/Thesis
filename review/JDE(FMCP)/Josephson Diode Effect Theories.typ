...
$ E_("ABS")(phi) approx p m Delta sqrt(1 - tau sin^2(phi/2)) + plunk.reduce vec(v)_F dot vec(q) $
where $tau$ is the channel transmission and $vec(v)_F$ is the Fermi velocity. The term $plunk.reduce vec(v)_F dot vec(q)$ explicitly breaks the symmetry between forward- and backward-propagating quasiparticles, leading to an asymmetric ABS spectrum, $E_("ABS")(phi) != E_("ABS")(-phi)$.
...
In a system with FMCP, an in-plane magnetic field (e.g., $B_y$, parallel to the current) can induce a finite Cooper pair momentum $q$. This leads to a spatially oscillating supercurrent density along the junction width, $j_s(x) prop sin(phi_0 + qx)$. The resulting interference pattern is the Fourier transform of this modulated current density, which deviates significantly from the standard Fraunhofer pattern. This "anomalous" Fraunhofer pattern exhibits two key signatures of FMCP:
...
where $alpha_R$ is the Rashba coupling strength, $sigma$ is the vector of Pauli matrices, and $vec(k)$ is the electron momentum. This interaction acts as a momentum-dependent effective magnetic field in the plane of the 2D system. Its primary effect is to lift the spin degeneracy of the electronic bands. A simple parabolic band $E(k) = plunk.reduce^2 k^2 / (2m^*)$ splits into two spin-chiral parabolic sub-bands with energies:
$ E_pm(k) = (plunk.reduce^2 k^2)/(2m^*) plus.minus alpha_R |k| $
...
- #strong[Magnetic Field Orientation:] The JDE exhibits strong magnetochiral anisotropy. For a system dominated by Rashba SOC, the effect is maximized when the in-plane magnetic field is oriented perpendicular to the direction of supercurrent flow ($vec(B) sect vec(I)$). The effect typically vanishes when the field is parallel to the current ($vec(B) parallel vec(I)$). This distinct angular dependence serves as a crucial experimental signature to identify the Rashba-SOC mechanism as the dominant contributor to the JDE.
- #strong[Material Parameters:] The diode effect is sensitive to the interplay between the characteristic energy scales of the system: the Rashba spin-orbit energy ($E_text(SO) = m^* alpha_R^2 / (2 plunk.reduce^2)$), the Zeeman energy ($E_Z = g mu_B B$), and the proximity-induced superconducting gap ($Delta^*$). The JDE is typically most pronounced when these energy scales are comparable, $E_text(SO) ~ E_Z ~ Delta^*$. Materials with a small effective mass $m^*$ and a large Landé g-factor $g^*$, such as InAs and InSb, are therefore ideal platforms for observing a strong JDE.
